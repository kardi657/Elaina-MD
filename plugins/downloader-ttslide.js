...
const cuki = async (m, { conn, text, usedPrefix, command }) => {
    ...
    for (let i = 0; i < slides.length; i++) {
      ...
      cards.push({
        body: proto.Message.InteractiveMessage.Body.fromObject({
          text: `✔️ Slide ${i + 1} dari ${slides.length}`
        }),
...
cuki.help = ['ttslide <url>'];
cuki.tags = ['downloader'];
cuki.command = /^ttslide$/i;
cuki.premium = false;
cuki.limit = 2;
